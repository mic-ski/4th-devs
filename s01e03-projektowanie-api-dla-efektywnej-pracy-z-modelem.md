...
**Porównanie MCP z narzędziami Shell / CLI**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_fs_shell-6ed06a27-0.png)</sub>

Grafika pokazuje kompromis między dwoma sposobami udostępniania agentowi AI pracy z systemem plików. Filesystem MCP opiera się na wielu wyspecjalizowanych narzędziach, co zwiększa rozmiar kontekstu, ale pozwala dokładnie kontrolować dostępne akcje, walidować parametry i ograniczać ryzyko błędnych operacji. Shell w sandboxie sprowadza interfejs do jednego narzędzia, np. `execute_shell`, dzięki czemu daje modelowi znacznie większą elastyczność i możliwość korzystania z istniejących komend CLI, takich jak `grep`, `find` czy `jq`. Kosztem tej swobody jest jednak większa odpowiedzialność po stronie sandboxa, który musi zabezpieczyć środowisko przed destrukcyjnymi lub niepożądanymi komendami.

W praktyce wybór między MCP a shellem jest decyzją projektową: **MCP premiuje kontrolę, przewidywalność i walidację**, natomiast **shell premiuje elastyczność, mniejszy narzut kontekstowy i większą sprawczość agenta**.
...
**Optymalizacja rodzajów narzędzi i ich odpowiedzialności**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/aidevs_4_mcp_optimization-090c65f0-6.png)</sub>

Grafika pokazuje proces redukowania liczby narzędzi udostępnianych modelowi. Po lewej stronie znajduje się oficjalny zestaw narzędzi Filesystem MCP, obejmujący trzynaście osobnych akcji, między innymi odczyt plików, zapis, edycję, listowanie katalogów, wyszukiwanie, przenoszenie oraz pobieranie metadanych. Po prawej stronie te same możliwości zostały pogrupowane w cztery szersze narzędzia: `search`, `read`, `write` oraz `manage`.

Najważniejszy wniosek z grafiki jest taki, że optymalizacja narzędzi dla LLM nie polega wyłącznie na mechanicznym zmniejszaniu ich liczby. Chodzi o znalezienie takiego poziomu abstrakcji, który ogranicza narzut kontekstowy, ale nadal zachowuje czytelne odpowiedzialności. Dobrze zaprojektowane narzędzie może łączyć kilka podobnych operacji, o ile model nadal jasno rozumie, kiedy i jak powinien go użyć.
...
**Szczegóły narzędzia FS Read**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_fsread-dacb713a-7.png)</sub>

Grafika przedstawia schemat narzędzia `fs_read`, które łączy odczyt plików i katalogów w jednym interfejsie. Wejście narzędzia zawiera prosty, ale elastyczny zestaw parametrów: ścieżkę, tryb działania, zakres linii, limity, głębokość drzewa katalogów, filtry typów plików, globy, wykluczenia oraz respektowanie `.gitignore`. Dzięki temu model może użyć tego samego narzędzia zarówno do odczytu konkretnego pliku, jak i do eksplorowania struktury katalogów.

Schemat odpowiedzi jest rozdzielony na przypadek pliku i katalogu. Dla pliku zwracana jest treść, checksum, liczba linii, zakres i informacja o przycięciu wyniku. Dla katalogu zwracane są wpisy, podsumowanie i statystyki. Istotne jest to, że odpowiedź zawiera tylko dane potrzebne agentowi do dalszej pracy, a opcjonalne pole `hint` może podpowiedzieć kolejny krok.
...
**Przykłady zachowań narzędzia fs_read, obsługi błędów i sytuacji brzegowych**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_fs_read_cases-7dfea0be-1.png)</sub>

Grafika pokazuje przykłady użycia `fs_read` w typowych i problematycznych sytuacjach. Widać między innymi zwykły odczyt pliku, automatyczne rozwiązanie niepełnej ścieżki, obsługę zbyt dużego pliku przez przycięcie wyniku oraz możliwość doczytania konkretnego zakresu linii. Dzięki temu narzędzie nie tylko zwraca dane, ale aktywnie pomaga agentowi kontynuować pracę bez zgadywania.

Szczególnie ważne są przykłady błędów: niejednoznaczna ścieżka zwraca listę kandydatów, a próba wyjścia poza dozwolony katalog zostaje zablokowana komunikatem `OUT_OF_SCOPE`. To pokazuje, że narzędzia dla LLM powinny być projektowane z myślą o korekcie typowych pomyłek modelu, a nie jedynie o poprawnej obsłudze idealnych zapytań.
...
**Schemat narzędzia fs_write**

<sub>[→ grafika](https://cloud.overment.com/2026-03-09/ai_devs_4_fs_write-4163dd85-b.png)</sub>

Grafika przedstawia schemat narzędzia `fs_write`, odpowiedzialnego za tworzenie i aktualizowanie plików. Wejście rozróżnia operację `create` oraz `update`, a w przypadku aktualizacji pozwala wybrać konkretną akcję, taką jak zastąpienie treści, wstawienie przed lub po wskazanych liniach albo usunięcie zakresu linii. Dzięki temu jedno narzędzie obsługuje kilka form modyfikacji pliku bez konieczności tworzenia wielu osobnych funkcji.

Najważniejsze elementy schematu to `checksum` oraz `dryRun`. Checksum pomaga zabezpieczyć plik przed nadpisaniem, jeśli jego treść zmieniła się od ostatniego odczytu, a `dryRun` pozwala modelowi zobaczyć planowaną zmianę bez jej wykonywania. To szczególnie istotne przy akcjach stratnych, gdzie błędna decyzja agenta może uszkodzić dane.
...
**Przykłady użycia fs_write**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_rswrite_cases-aee30a3e-9.png)</sub>

Grafika pokazuje trzy scenariusze użycia `fs_write`: utworzenie pliku, podgląd aktualizacji w trybie `dryRun` oraz błąd spowodowany niezgodnością checksum. W pierwszym przypadku aplikacja zwraca informację o utworzonym pliku, nowy checksum i hint potwierdzający sukces. W drugim narzędzie nie modyfikuje pliku, lecz zwraca status `preview`, diff i podpowiedź, aby uruchomić operację ponownie z `dryRun=false`, jeśli zmiana ma zostać zastosowana.

Najważniejszy jest trzeci przykład, w którym narzędzie odmawia aktualizacji, ponieważ plik zmienił się od ostatniego odczytu. Zamiast ogólnego błędu pojawia się kod `CHECKSUM_MISMATCH` oraz wskazówka, aby ponownie wczytać plik przez `fs_read`. To pokazuje, jak projektować odpowiedzi błędów, które pomagają agentowi samodzielnie wrócić na poprawną ścieżkę.
...
**Schemat narzędzia fs_search**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_fs_search-f5ebca01-4.png)</sub>

Grafika przedstawia schemat narzędzia `fs_search`, które pozwala przeszukiwać zarówno nazwy plików, jak i ich zawartość. Parametry wejściowe obejmują ścieżkę, zapytanie, cel wyszukiwania (`filename`, `content` lub `all`), tryb dopasowania (`literal`, `regex`, `fuzzy`) oraz ustawienia takie jak wielkość liter, dopasowanie całych słów, wieloliniowość, typy plików, globy, wykluczenia, głębokość i maksymalna liczba wyników.

Odpowiedź rozdziela wyniki dopasowań plików od dopasowań treści. Dzięki temu model może najpierw szybko znaleźć potencjalnie istotne pliki, a następnie zdecydować, które z nich odczytać. To ważny wzorzec projektowy: wyszukiwanie nie powinno od razu zalewać kontekstu pełną treścią dokumentów, lecz zwracać uporządkowane, ograniczone wyniki przydatne do kolejnych kroków.
...
**Schemat narzędzia fs_manage**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_fs_manage-5a927df9-b.png)</sub>

Grafika przedstawia schemat narzędzia `fs_manage`, które grupuje operacje zarządzania plikami i katalogami: usuwanie, zmianę nazwy, przenoszenie, kopiowanie, tworzenie katalogu oraz pobieranie metadanych. Wejście narzędzia składa się z operacji, ścieżki źródłowej, opcjonalnej ścieżki docelowej i flagi `force`, która pozwala kontrolować nadpisywanie istniejących zasobów.

Ważnym elementem projektu jest ograniczenie destrukcyjnych operacji. Grafika zaznacza, że katalogi muszą być puste, aby można je było usunąć, a operacje rekurencyjne nie są wspierane. To świadomy kompromis między możliwościami agenta a bezpieczeństwem danych. Narzędzie daje modelowi praktyczną sprawczość, ale ogranicza zakres szkód, jakie mogłyby wynikać z błędnej decyzji.
...
**Wskazówki dla LLM w odpowiedziach narzędzi**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_hints_llm-16782f1c-6.png)</sub>

Grafika pokazuje zasady projektowania komunikatów `hint` i błędów z myślą o modelach językowych. Zamiast zwracać ogólne komunikaty typu „operation failed" lub „target exists", narzędzie powinno podawać konkretne działania naprawcze, np. odczytanie katalogu nadrzędnego, użycie `force=true` albo ponowne wywołanie `fs_read` w celu uzyskania aktualnego checksum.

Drugi ważny wniosek dotyczy osadzania konkretnych wartości w odpowiedziach. Jeśli plik został utworzony, warto zwrócić jego checksum. Jeśli zakres linii jest błędny, warto poinformować, ile linii faktycznie ma plik. Takie komunikaty mogą zmniejszać potrzebę zgadywania, prowadzić agenta przez kolejne kroki i ograniczać liczbę niepotrzebnych wywołań narzędzi.
...
**Złożoność dynamicznych hintów i komunikatów**

<sub>[→ grafika](https://cloud.overment.com/2026-01-27/ai_devs_4_hint_complexity-4767c8b8-b.png)</sub>

Grafika ilustruje koszt projektowania odpowiedzi narzędzi zoptymalizowanych pod pracę z LLM. Dynamiczne komunikaty, szczegółowe statusy, wskazówki naprawcze i obsługa sytuacji brzegowych wymagają bardziej rozbudowanej logiki niż klasyczne, generyczne odpowiedzi API. Narzędzie musi nie tylko wykonać operację, ale także zrozumiale opisać jej rezultat, ograniczenia oraz możliwe dalsze kroki.

Najważniejszy wniosek jest taki, że przy projektowaniu narzędzi dla agentów złożoność często przenosi się z interfejsu użytkownika do warstwy kodu. Im bardziej chcemy pomagać modelowi działać samodzielnie, tym więcej przypadków musimy przewidzieć po stronie implementacji. Warto jednak uwzględnić ten koszt już na etapie planowania narzędzi.
...
<sub>[→ grafika](https://cloud.overment.com/2026-01-28/ai_devs_4_mcp_concepts-2bc4062d-d.png)</sub>

Grafika wyjaśnia podstawową architekturę Model Context Protocol: host, client i server. Hostem jest aplikacja użytkownika, która zawiera model językowy oraz tworzy klientów MCP. Każdy klient odpowiada za połączenie z jednym serwerem MCP, który działa jako zewnętrzny proces lub zdalna usługa. Połączenie może odbywać się lokalnie przez STDIO albo zdalnie przez Streamable HTTP.

Kluczowy wniosek z grafiki jest taki, że serwer MCP nie jest częścią samego modelu ani hosta. To osobny komponent, który udostępnia narzędzia, zasoby lub inne funkcje. Host może jednocześnie utrzymywać wiele klientów i łączyć się z wieloma serwerami, np. Linear, Google Calendar czy Google Maps. MCP porządkuje więc sposób dostarczania integracji do aplikacji korzystających z LLM.
...
**Połączenie natywnych narzędzi z narzędziami MCP**

<sub>[→ grafika](https://cloud.overment.com/2026-01-28/ai_devs_4_mcp_tools-890847b5-8.png)</sub>

Grafika pokazuje, że narzędzia natywne aplikacji i narzędzia pochodzące z serwera MCP mogą zostać połączone w jeden wspólny zestaw dla modelu. Po stronie aplikacji istnieją lokalne definicje i handlery, np. `calculate`, a po stronie serwera MCP zewnętrzne narzędzia, np. `get_weather`. Klient MCP pobiera schematy z serwera i konwertuje je do formatu zgodnego z mechanizmem tool calling.

Najważniejsze jest to, że z perspektywy modelu pochodzenie narzędzia nie musi mieć znaczenia. Wszystkie schematy trafiają do jednego żądania, a w czasie wykonania aplikacja odnajduje właściwy handler: lokalny albo MCP. Dzięki temu MCP nie zastępuje natywnych narzędzi, lecz może z nimi współistnieć jako dodatkowa warstwa dostarczania integracji.
...
**Przykład interakcji z agentem wyposażonym w narzędzia natywne oraz MCP**

<sub>[→ grafika](https://cloud.overment.com/2026-01-28/ai_devs_4_mcp_interaction-7f4d92bc-0.png)</sub>

Grafika przedstawia przebieg działania agenta, który korzysta zarówno z narzędzia natywnego, jak i narzędzia MCP. W pierwszym przykładzie użytkownik prosi o obliczenie działania matematycznego, agent wybiera lokalne narzędzie `calculate`, kod zwraca wynik, a agent formułuje odpowiedź. W drugim przykładzie użytkownik pyta o pogodę w Tokio, agent wywołuje narzędzie `get_weather` pochodzące z MCP, a następnie odpowiada na podstawie zwróconych danych.

Wniosek z grafiki jest taki, że agent nie musi traktować narzędzi MCP jako czegoś jakościowo innego od narzędzi natywnych. Jeśli host dobrze połączy schematy i handlery, model widzi po prostu listę dostępnych możliwości. Różnica dotyczy głównie miejsca implementacji i sposobu dostarczenia narzędzia.
...
<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_mcp_apps-fb210306-c.png)</sub>

Grafika pokazuje przykład bogatej odpowiedzi narzędzia MCP, która nie ogranicza się do tekstu lub JSON-a. Użytkownik prosi o pokazanie tygodniowego przychodu, model decyduje o wywołaniu narzędzia `get_weekly_revenue`, klient przekazuje żądanie do serwera, a serwer zwraca odpowiedź typu `image`. Następnie host renderuje wynik jako wykres widoczny dla użytkownika.

Najważniejszy wniosek jest taki, że MCP może służyć nie tylko do wykonywania akcji, ale także do dostarczania interaktywnych lub wizualnych wyników w aplikacji hosta. Odpowiedź narzędzia może zostać przetworzona przez klienta i zaprezentowana w formie wygodniejszej niż surowe dane tekstowe.
...
<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_mcp_resources-e3c48eb8-a.png)</sub>

Grafika pokazuje mechanizm zasobów MCP, czyli danych dostępnych do odczytu. Użytkownik otwiera selektor zasobów w interfejsie hosta, klient pobiera listę zasobów z serwera, a użytkownik wybiera konkretny plik, np. `config.json`. Następnie klient odczytuje zasób przez jego URI, a zawartość zostaje wstrzyknięta do kontekstu modelu.

W odróżnieniu od narzędzi, zasoby nie muszą oznaczać wykonania akcji. Są sposobem na udostępnianie modelowi danych, takich jak pliki, konfiguracje, dokumenty lub listy obiektów. Ważne jest też to, że aktywacja zasobu może zależeć od interfejsu użytkownika, a nie wyłącznie od decyzji agenta. Dzięki temu użytkownik lub host może kontrolować, jakie dane trafiają do kontekstu.
...
<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_mcp_prompts-dd7e84ce-0.png)</sub>

Grafika przedstawia mechanizm promptów MCP, czyli predefiniowanych instrukcji udostępnianych przez serwer. Użytkownik otwiera selektor promptów w interfejsie hosta, klient pobiera ich listę z serwera, a użytkownik wybiera np. `code_review`. Następnie klient pobiera konkretny prompt wraz z argumentami, takimi jak język programowania, i ładuje instrukcję do kontekstu modelu.

Najważniejszy wniosek jest taki, że prompty MCP nie są narzędziami wykonywanymi przez agenta, ale gotowymi szablonami pracy, które użytkownik może aktywować. Pozwalają standaryzować powtarzalne zadania, takie jak review kodu, streszczanie czy wyjaśnianie błędów. Dzięki argumentom mogą być dynamiczne, ale nadal pozostają kontrolowanymi instrukcjami dostarczanymi przez serwer MCP.
...
<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_mcp_sampling-cb5c8953-8.png)</sub>

Grafika pokazuje mechanizm `sampling`, czyli odwrócony przepływ komunikacji, w którym to serwer MCP prosi hosta o użycie modelu językowego. Przykładowo serwer przetwarza dane i w trakcie operacji potrzebuje klasyfikacji wyniku przez LLM. Wysyła więc żądanie `sampling/createMessage`, klient przekazuje je do hosta, model generuje odpowiedź, a serwer kontynuuje działanie z uzyskanym rezultatem.

Kluczowe jest to, że sampling odwraca typowy kierunek interakcji. Zwykle model wywołuje narzędzie, ale tutaj narzędzie lub serwer prosi o skorzystanie z modelu. Taki mechanizm może być przydatny przy długich operacjach, klasyfikacji, ekstrakcji lub podejmowaniu decyzji po stronie serwera, ale wymaga kontroli hosta i akceptacji użytkownika, ponieważ serwer nie powinien samowolnie wysyłać treści do modelu.
...
<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_elicitation-305381f2-4.png)</sub>

Grafika przedstawia mechanizm `elicitation`, w którym serwer MCP prosi użytkownika o dodatkowe dane lub potwierdzenie w trakcie wykonywania operacji. Przykład dotyczy wdrożenia na produkcję: serwer zatrzymuje wykonanie, wysyła żądanie utworzenia formularza z pytaniem o potwierdzenie, host renderuje interfejs użytkownika, a użytkownik podaje dane, np. środowisko `production` i zgodę `yes`.

Najważniejszy wniosek jest taki, że elicitation pozwala serwerowi MCP włączyć człowieka w środek procesu bez opuszczania hosta. Jest to szczególnie ważne przy akcjach wrażliwych, takich jak deployment, autoryzacja, płatności lub operacje wymagające potwierdzenia. W odróżnieniu od samplingu, tutaj odwrócona komunikacja nie trafia do modelu, lecz bezpośrednio do użytkownika.
...
**Porównanie workflow z agentem w kontekście tłumaczeń**

<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_workflow_vs_agent-de39b393-d.png)</sub>

Grafika porównuje klasyczny workflow z podejściem agentowym na przykładzie tłumaczenia dokumentu. Workflow składa się z liniowej sekwencji: podziel dokument, przetłumacz fragmenty, połącz wynik. Taki proces jest przewidywalny i łatwy do kontrolowania, ale ma ograniczoną elastyczność, ponieważ nie przewiduje swobodnego powrotu do wcześniejszych kroków ani samodzielnej korekty błędów przez model.

W podejściu agentowym centralnym elementem jest pętla `Decide → Act`, w której agent może czytać, pisać, tłumaczyć i sprawdzać wynik w dowolnej kolejności. Działanie powtarza się aż do zakończenia zadania. Wniosek jest taki, że workflow daje większą kontrolę, natomiast agent daje większą adaptacyjność i możliwość samokorekty, ale wymaga lepszego projektowania narzędzi oraz zabezpieczeń.
...
**Architektura agenta do tłumaczeń**

<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_translation_agent-301a7de1-2.png)</sub>

Grafika przedstawia architekturę backendowego agenta tłumaczącego dokumenty z użyciem MCP. W aplikacji znajdują się trzy główne elementy: `File Watcher`, który obserwuje katalog i wykrywa nowe pliki, `Agentic Loop`, która podejmuje decyzje i wykonuje działania, oraz `MCP Client`, który integruje aplikację z serwerem MCP. Poza aplikacją znajdują się system plików z katalogami `translate` i `translated` oraz zewnętrzny proces `Files MCP Server`.

Najważniejszy wniosek jest taki, że host MCP nie musi być aplikacją graficzną. Może być backendowym procesem, który uruchamia klienta MCP i korzysta z narzędzi serwera do pracy z plikami. MCP Client jest częścią aplikacji, a serwer MCP działa osobno i udostępnia ograniczony, kontrolowany dostęp do workspace.
...
<sub>[→ grafika](https://cloud.overment.com/2026-01-29/ai_devs_4_mcp_connection-7a4e6f4d-0.png)</sub>

Grafika pokazuje trzyetapowy proces połączenia klienta MCP z serwerem. Najpierw aplikacja wczytuje konfigurację z pliku `mcp.json` i wybiera ustawienia konkretnego serwera. Następnie tworzy klienta MCP oraz transport STDIO, przekazując komendę i argumenty potrzebne do uruchomienia procesu serwera. Ostatni etap to użycie połączenia: pobranie listy narzędzi oraz wykonywanie wywołań, np. `fs_read` na pliku w katalogu tłumaczeń.

Wniosek z grafiki jest taki, że integracja MCP po stronie backendu składa się z relatywnie prostych kroków: konfiguracja, połączenie i użycie. Jednocześnie klient pełni ważną rolę pośrednika między pętlą agenta a zewnętrznym serwerem, ukrywając szczegóły transportu i pozwalając traktować narzędzia MCP podobnie jak lokalne funkcje.
...
**OAuth w serwerach MCP**

<sub>[→ grafika](https://cloud.overment.com/2026-01-30/ai_devs_4_mcp_security-2da663fe-5.png)</sub>

Grafika przedstawia przepływ OAuth po stronie serwera MCP. Proces zaczyna się od discovery, czyli pobrania metadanych protected resource oraz authorization server. Następnie klient pobiera metadane, wykonywana jest walidacja połączenia, a później serwer wydaje kod autoryzacyjny, uwzględniając między innymi PKCE, redirect URI i sprawdzenie wersji protokołu. Kolejny etap to wymiana kodu na tokeny oraz ich bezpieczne przechowywanie.

Najważniejszy wniosek jest taki, że OAuth w MCP nie sprowadza się do prostego przekazania tokena. Serwer musi obsłużyć wiele etapów: discovery, walidację klienta, wydanie kodu, wymianę na tokeny, szyfrowanie, odświeżanie i egzekwowanie uprawnień przy wywołaniach API. To znacząco zwiększa złożoność serwera, ale jest wymagane wszędzie tam, gdzie serwer MCP korzysta z OAuth jako mechanizmu autoryzacji.
...
**OAuth dla MCP Host/Client**

<sub>[→ grafika](https://cloud.overment.com/2026-01-30/ai_devs_4_mcp_host_security-b5d29927-d.png)</sub>

Grafika pokazuje odpowiedzialności OAuth po stronie hosta i klienta MCP. Proces obejmuje implementację providera OAuth w aplikacji hosta, bezpieczne przechowywanie tokenów i danych PKCE, pobieranie metadanych serwera, walidację HTTPS, obsługę redirect URI, generowanie PKCE i parametru `state`, wymianę kodu na tokeny oraz późniejsze używanie i odświeżanie tokenów przy wywołaniach API.

Wniosek z grafiki jest taki, że obsługa OAuth nie dotyczy wyłącznie serwera MCP. Host i klient również muszą poprawnie przeprowadzić użytkownika przez proces autoryzacji, zadbać o bezpieczeństwo tokenów i pilnować poprawności całego przepływu. To ważne szczególnie wtedy, gdy budujemy własnego hosta MCP lub backendową aplikację, która działa jak host bez klasycznego interfejsu graficznego.
...
**Konfiguracja Cloudflare Workers dla MCP**

<sub>[→ grafika](https://cloud.overment.com/2026-01-30/ai_devs_4_cloudflare-51ea7f86-1.png)</sub>

Grafika przedstawia proces publikacji zdalnego serwera MCP na Cloudflare Workers. Pierwszy etap to instalacja i inicjalizacja projektu, następnie utworzenie przestrzeni KV na tokeny oraz skopiowanie identyfikatora namespace do konfiguracji `wrangler.toml`. Kolejny krok obejmuje ustawienie nazwy serwera, powiązania KV, zmiennych środowiskowych oraz ewentualnych sekretów OAuth, takich jak client ID, client secret czy klucz szyfrowania tokenów.

Na końcu proces sprowadza się do wykonania deploymentu przez `wrangler deploy`, po którym serwer MCP jest dostępny pod publicznym adresem `/mcp`. Grafika przypomina też o limitach darmowego planu, między innymi liczbie requestów i operacji KV. Wniosek jest taki, że Cloudflare Workers upraszcza publikację MCP, ale wymaga świadomej konfiguracji sekretów, storage'u i kosztów.
...
**Podstawowa konfiguracja NGINX dla Remote MCP**

<sub>[→ grafika](https://cloud.overment.com/2026-01-30/ai_devs_4_nginx-959684a3-6.png)</sub>

Grafika pokazuje, jak wystawić lokalny serwer MCP przez NGINX jako reverse proxy. Architektura zakłada, że klient łączy się z NGINX po HTTPS na porcie 443, a NGINX przekazuje ruch do lokalnego serwera MCP działającego na porcie 3000. W prostym wariancie bez OAuth wystarczy obsłużyć endpoint `/mcp`, który jest mapowany na lokalny adres serwera.

W wariancie z OAuth konfiguracja musi uwzględniać dodatkowe ścieżki, takie jak `/.well-known/*`, `/authorize`, `/token`, `/register` oraz `/oauth/callback`. Grafika pokazuje też podstawowe elementy konfiguracji `nginx.conf`: certyfikaty SSL, `server_name`, `proxy_pass` i nagłówki proxy. Wniosek jest taki, że publikacja Remote MCP przez VPS i NGINX jest technicznie prosta, ale OAuth znacząco zwiększa liczbę wymaganych endpointów.
